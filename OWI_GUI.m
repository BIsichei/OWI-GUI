...
%EMERGENCY STOP
    function Stop(~,~)
        if Data.Set 
            for i = 1:5
                digitalWrite(Arduino,Data.Pins(i,3), ~Data.Pins(6,1))
                digitalWrite(Arduino,Data.Pins(i,2), ~Data.Pins(6,1))
            end
        else
...
%MOVE ARM
    function move(val,goal)
        Pin = getDir(val,goal);
        IdlePins = isnan(Pin);
        temp = sub2ind(size(Data.Pins), val, Pin);
        MovePin = Data.Pins(temp)';
        if sum(~IdlePins) && ~(strcmp(goal,'+')||strcmp(goal,'-'))
            digitalWrite(Arduino,MovePin(~IdlePins),Data.Pins(6,1)*ones(numel(find(~IdlePins)),1));
            n = numel(find(IdlePins));
            while n < numel(Pin)
                pause(0.02);
                Pin = getDir(val,goal);
                NewIdlePins = isnan(Pin);
                n = numel(find(NewIdlePins));
                NewIdlePins = xor(NewIdlePins,IdlePins);
                if sum(NewIdlePins)
                    digitalWrite(Arduino,MovePin(NewIdlePins),~Data.Pins(6,1)*ones(numel(find(NewIdlePins)),1));
                end
            ...
            for i = 1:numel(val)
                if strcmp(goal,'+')||strcmp(goal,'-')
                    digitalWrite(Arduino,MovePin(i),Data.Pins(6,1));
                else
                    EmStop(Arduino,Data.Pins,i);
                end
            end            
        end
    end
